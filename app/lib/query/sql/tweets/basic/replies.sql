/*
    Select tweets and their authors. For tweets which are replies, show which tweet and profile they were directed at.

    Optionally filter to only replies with
         WHERE Tweet.in_reply_to_profile_guid IS NOT NULL

    We do left outer join and not an inner join, to show gaps where Profile GUID is set but Profile screen name is missing because the Profile is not in the db yet.
*/
