...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="""Fetch Tweets for Profiles utility. Filter Profiles
            ...
            Tweets are assigned to the '{0}' Campaign.""".format(
            UTILITY_CAMPAIGN
        )
    )
    ...
    updateGrp = parser.add_argument_group(
        "Update", "Create or update Tweet" " records."
    )
    ...
    if args.available:
        ...
        categoryResult = db.Category.select(IN(db.Category.q.name, inputCategories))
        ...
        profResults = db.Profile.select(
            AND(db.Profile.j.categories, IN(db.Category.q.name, inputCategories))
        )
        ...
        try:
            ...
            campaignRec = db.Campaign(name=UTILITY_CAMPAIGN, searchQuery=None)
        ...
        if profCount:
            ...
            raise ValueError(
                "No profiles were found in the categories"
